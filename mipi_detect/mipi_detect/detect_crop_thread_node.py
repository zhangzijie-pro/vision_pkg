...
参数(ros2 run mipi_detect mutil_thread_crop_detect_node --ros-args -p key:=val ...):
  multi_threading: bool = True      # 是否启用多线程
  crop_edges: bool = False          # 是否需要裁剪边缘部分
  crop_h_ratio: float = 0.8         # 裁剪高度比例
  crop_w_ratio: float = 1           # 裁剪宽度比例
  crop_resize: bool = False         # 裁剪后是否需要缩放到模型输入大小
...
class Detect(Node):
    def __init__(self, name="mipi_detect_node"):
        ...
        self.declare_parameter('publisher_node_name', '/yolo_detections')
        ...
        self.publisher_topic = self.get_parameter('publisher_node_name').value
